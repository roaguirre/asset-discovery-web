import type { CreateRunPayload, LiveAssetRow, LiveAuthSession, LiveEventRecord, LivePivotRecord, LiveRunRecord, LiveTrace } from './types'
...
export interface LiveAppDeps {
  ...
  subscribeRuns(ownerUID: string, onRuns: (runs: LiveRunRecord[]) => void): Unsubscribe
  ...
  decidePivot(runID: string, pivotID: string, decision: 'accepted' | 'rejected'): Promise<LivePivotRecord>
}
