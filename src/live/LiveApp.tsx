...
export default function LiveApp({ deps }: LiveAppProps) {
  ...
  const [route, setRoute] = useState<LiveRouteState>(() => parseLiveHash(window.location.hash))
  ...
  const visibleAssets = filterAssets(assets, deferredAssetSearch, activeAssetTab)
    .slice()
    .sort((left, right) => compareAssetRows(left, right, activeAssetSort))
  ...
  const auditPivots = pivots
    ...
    .slice()
    .sort((left, right) => comparePivots(left, right))
  const traceTargetID = resolveTraceTargetID(route.view, route.assetID, traces, assets)
  const selectedTrace = traces.find((trace) => trace.asset_id === traceTargetID) ?? null
  const selectedTraceAsset = assets.find((asset) => asset.asset_id === traceTargetID) ?? null
  ...
  const viewItems: ViewNavigationItem[] = [
    ...
    { view: 'pivots', label: viewLabels.pivots, count: pendingPivots.length + auditPivots.length },
  ...
  const drawerToggleLabel = compactViewport
    ...
    : expandedDrawer
      ? 'Collapse navigation'
      : 'Expand navigation'
  const runSelectorLabel = selectedRun ? formatRunSelectorLabel(selectedRun) : runsHydrated ? 'No runs yet' : 'Loading runs'
  ...
  useEffect(() => {
    window.localStorage.setItem(drawerPreferenceKey, drawerCollapsed ? 'true' : 'false')
  }, [drawerCollapsed])
  ...
  useEffect(() => {
    ...
    if (runs.length === 0) {
      if (route.runID || route.assetID || route.pivotID || route.view !== 'assets' || route.assetTab !== 'all') {
        setRoute(emptyLiveRoute())
      }
      return
    }
  ...
  }, [route.assetID, route.assetTab, route.pivotID, route.runID, route.view, runs, runsHydrated, session?.uid])
  ...
  useEffect(() => {
    if (route.view !== 'trace' || !traceTargetID || route.assetID === traceTargetID) {
      return
    }
    setRoute((current) => ({
      ...current,
  ...
  useEffect(() => {
    if (!drawerOpen && !createModalOpen && !accountMenuOpen && !runMenuOpen) {
      return
    }
  ...
  }, [accountMenuOpen, createModalOpen, drawerOpen, runMenuOpen])

  useEffect(() => {
    if (!accountMenuOpen && !runMenuOpen) {
      return
    }
  ...
  async function handleCreateRun(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setModalMessage(error instanceof Error ? error.message : 'Failed to create run.')
    } finally {
      setBusyAction('')
    }
  }

  async function handlePivotDecision(pivot: LivePivotRecord, decision: 'accepted' | 'rejected') {
    setBusyAction(`${decision}:${pivot.id}`)
    setMessage('')
    try {
      await deps.decidePivot(route.runID, pivot.id, decision)
      setMessage(`${decision === 'accepted' ? 'Accepted' : 'Rejected'} ${pivot.root}.`)
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Failed to update pivot.')
    } finally {
      setBusyAction('')
    }
  }
  ...
  function handleSortChange(key: AssetSortKey) {
    setAssetSort((current) => toggleAssetSort(normalizeAssetSort(current, activeAssetTab), key))
  }
  ...
  function handleTooltipOver(event: MouseEvent<HTMLElement> | FocusEvent<HTMLElement>) {
    ...
    return (
      <main className="live-shell live-auth-shell">
        <section className="live-auth-card">
          ...
          <h1>Live asset discovery, pivot review, and trace context in one workspace.</h1>
          <p className="live-intro">
            Sign in with Google to launch automated or manual runs, review recommended pivots, and watch discovery state stream
            into the client.
          </p>
          <button type="button" className="hero-button" onClick={handleSignIn} disabled={busyAction === 'sign-in'}>
            {busyAction === 'sign-in' ? 'Opening Google...' : 'Sign In With Google'}
          </button>
          <p className="meta-note">Approved verified Google accounts only.</p>
          {message ? <p className="status-line status-error">{message}</p> : null}
        </section>
      </main>
    )
  }
    ...
    return (
      <main className="live-shell live-auth-shell">
        <section className="live-auth-card">
          ...
          <h1>{session.email}</h1>
          <p className="live-intro">This demo only accepts approved verified accounts. Sign out and switch to an authorized login.</p>
          <button type="button" className="hero-button" onClick={handleSignOut} disabled={busyAction === 'sign-out'}>
  ...
  return (
    ...
    >
      <header className="workspace-topbar">
        <div className="topbar-left">
          ...
          <div className="run-selector-shell" ref={runMenuRef}>
            ...
            >
              <span className="run-selector-label" title={selectedRun?.id || runSelectorLabel}>
                {runSelectorLabel}
              </span>
              <ChevronDownIcon />
            </button>
            {runMenuOpen ? (
              <section className="run-selector-menu" role="menu" aria-label="Run selector">
                {runs.map((run) => (
                  <button
                    key={run.id}
                    type="button"
                  ...
                  >
                    <div className="run-selector-item-copy">
                      ...
                      <span>{run.mode === 'manual' ? 'Manual review enabled' : 'Autonomous execution'}</span>
                    </div>
      ...
      <div className="live-body">
          ...
          <div className="drawer-inner">
              ...
              <div className="drawer-header">
                <div className="drawer-brand">
                  <div>
                    ...
                    <p className="drawer-copy">Runs, traces, pivots, and the live asset table stay in one shell.</p>
                  </div>
                </div>
              </div>
            ) : null}
            ...
            <nav className="drawer-nav" aria-label="Workspace views">
              {viewItems.map((item) => (
                ...
                >
                  <span className="drawer-link-icon">{iconForView(item.view)}</span>
                  {expandedDrawer ? <span className="drawer-link-label">{item.label}</span> : null}
                  {expandedDrawer ? <strong className="drawer-link-count">{item.count}</strong> : null}
                </button>
              ))}
            </nav>
        ...
        <section className="live-main">
          ...
          {selectedRun ? (
            ...
            <section className="panel run-overview">
              <div className="panel-heading">
                ...
                <span className={`status-pill status-${selectedRun.status}`}>{formatStatus(selectedRun.status)}</span>
              </div>
              <div className="stat-grid">
                <StatCard label="Mode" value={humanizeToken(selectedRun.mode, 'n/a')} />
                <StatCard label="Wave" value={String(selectedRun.current_wave)} />
                <StatCard label="Assets" value={String(selectedRun.asset_count)} />
                <StatCard label="Pending Pivots" value={String(selectedRun.pending_pivot_count)} />
              </div>
              {selectedRun.last_error ? <p className="status-line status-error">{selectedRun.last_error}</p> : null}
            </section>
              ...
              <section className="panel view-panel asset-view-panel">
                <div className="asset-toolbar">
                  <div>
                    ...
                    <h2>{visibleAssets.length} visible</h2>
                    <p className="panel-copy">Sort, filter, and inspect the live Firestore projection without moving pivots or trace out of the way.</p>
                  </div>
                  <div className="asset-toolbar-controls">
                    ...
                    <input
                      ...
                      placeholder="Search identifiers, sources, ownership, or notes"
                      value={assetSearch}
                ...
                <div className="asset-table-shell">
                  {visibleAssets.length > 0 ? (
                    ...
                    <p className="empty-copy table-empty-copy">No assets match the current filter.</p>
                  )}
                </div>
              </section>
            ) : null}
              ...
              <section className="panel view-panel trace-view">
                <div className="panel-heading">
                  <div>
                    ...
                    <h2>{selectedTrace?.identifier || selectedTraceAsset?.identifier || 'No trace selected yet'}</h2>
                    <p className="panel-copy">Open any asset from the table to carry its context into the dedicated trace view.</p>
                  </div>
                  <label className="trace-select">
                    Focus asset
                    <select value={traceTargetID} onChange={(event) => handleSelectTraceAsset(event.target.value)} disabled={traceOptions.length === 0}>
                      {traceOptions.length === 0 ? <option value="">No assets available</option> : null}
                ...
                {traceTargetID ? (
                  <div className="trace-grid">
                    <article className="trace-card">
                      ...
                      <h3>{selectedTrace?.identifier || selectedTraceAsset?.identifier || 'Unknown asset'}</h3>
                      <div className="badge-row">
                        <span className="pill">{humanizeToken(selectedTrace?.asset_type || selectedTraceAsset?.asset_type, 'Unknown')}</span>
                        {selectedTrace?.domain_kind ? <span className="pill pill-subtle">{humanizeToken(selectedTrace.domain_kind)}</span> : null}
                        {selectedTrace?.registrable_domain ? <span className="pill pill-subtle">{selectedTrace.registrable_domain}</span> : null}
                        {selectedTrace?.resolution_status ? <span className="pill pill-subtle">{humanizeToken(selectedTrace.resolution_status)}</span> : null}
                      </div>
                      <section className="trace-section-stack">
                        {(selectedTrace?.sections ?? []).slice(0, 4).map((section) => (
                        ...
                        {(selectedTrace?.sections ?? []).length === 0 ? (
                          <p className="empty-copy">Trace sections will appear here as checkpoint snapshots refresh.</p>
                        ) : null}
                      </section>
                    </article>

                    <article className="trace-card">
                      <p className="eyebrow">Trace Nodes</p>
                      <h3>{selectedTrace?.nodes?.length ?? 0} exported nodes</h3>
                      <ul className="trace-node-list">
                      ...
                      {(selectedTrace?.nodes ?? []).length === 0 ? <p className="empty-copy">No trace nodes were exported for this asset yet.</p> : null}
                    </article>

                    <article className="trace-card">
                      <p className="eyebrow">Related Assets</p>
                      <h3>{selectedTrace?.related?.length ?? 0} linked asset{selectedTrace?.related?.length === 1 ? '' : 's'}</h3>
                      <div className="related-list">
                        {(selectedTrace?.related ?? []).map((link) => (
                          <button key={link.asset_id} type="button" className="ghost-button compact related-link" onClick={() => handleOpenTrace(link.asset_id)}>
                            <strong>{link.identifier}</strong>
                            <span>{link.label}</span>
                          </button>
                        ))}
                      </div>
                      {(selectedTrace?.related ?? []).length === 0 ? <p className="empty-copy">No related assets were linked for this trace snapshot.</p> : null}
                    </article>
                  </div>
                ) : (
                  <p className="empty-copy">Trace snapshots will appear here once the selected run exports asset context.</p>
                )}
              </section>
            ) : null}
              ...
              <section className="panel view-panel">
                <div className="panel-heading">
                  <div>
                    ...
                    <h2>{pendingPivots.length} pending review</h2>
                    <p className="panel-copy">Pending decisions stay at the top. Automated outcomes remain visible in the audit lane.</p>
                  </div>
                </div>
                <div className="pivot-columns">
                  <section className="pivot-column">
                    ...
                    <div className="pivot-list">
                      {pendingPivots.map((pivot) => (
                        ...
                        >
                          <div className="pivot-copy">
                            ...
                            <p>{pivot.recommendation_reason || 'AI recommended this pivot without an explicit explanation.'}</p>
                            <div className="pivot-meta">
                              <span>{pivot.collector || 'judge'}</span>
                              <span>{pivot.seed_label || pivot.seed_domains?.join(', ') || 'Seed context pending'}</span>
                      ...
                      {pendingPivots.length === 0 ? <p className="empty-copy">No pivots need review right now.</p> : null}
                    </div>
                  </section>
                  ...
                  <section className="pivot-column">
                    ...
                    <div className="audit-list">
                      {auditPivots.map((pivot) => (
                        <article key={pivot.id} className="audit-item">
                          <div className="audit-header">
                            <strong>{pivot.root}</strong>
                            <span className={`status-pill status-${pivot.status}`}>{formatStatus(pivot.status)}</span>
                          </div>
                          <p>{pivot.recommendation_reason || 'No explicit rationale was stored for this recommendation.'}</p>
                          <div className="pivot-meta">
                            <span>{pivot.collector || 'judge'}</span>
                      ...
                      {auditPivots.length === 0 ? <p className="empty-copy">Resolved recommendations will land here.</p> : null}
                    </div>
                  </section>
                </div>
              </section>
              ...
              <section className="panel view-panel">
                <div className="panel-heading">
                  <div>
                    ...
                    <h2>{events.length} recent event{events.length === 1 ? '' : 's'}</h2>
                    <p className="panel-copy">The feed stays isolated from assets, pivots, and trace so the main workspace can stay focused.</p>
                  </div>
                </div>
                <div className="activity-list">
                  {events.map((event) => (
                    <article key={event.id} className="activity-item">
                      <span className="status-pill subtle">{humanizeToken(event.kind, event.kind)}</span>
                      <strong>{event.message}</strong>
                      <span>{formatDate(event.created_at)}</span>
                    </article>
                  ))}
                  {events.length === 0 ? <p className="empty-copy">Run events will appear here once execution begins.</p> : null}
                </div>
              </section>
            ) : null}
          </>
          ...
          <section className="panel empty-panel">
            ...
            <p className="empty-copy">The live workspace hydrates from Firestore once a signed-in creator owns at least one run.</p>
            <button type="button" className="hero-button" onClick={openCreateModal}>
              Create Run
            </button>
          </section>
        )}
      ...
      {createModalOpen ? (
        ...
        >
          <section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="new-run-title">
            <div className="panel-heading">
              <div>
                <p className="eyebrow">New Run</p>
                <h2 id="new-run-title">Plant seeds</h2>
                <p className="panel-copy">Use a compact seed set, then let the workspace take over.</p>
              </div>
              <button type="button" className="ghost-button compact" onClick={() => setCreateModalOpen(false)}>
                Close
              </button>
            </div>
            <form className="create-form" onSubmit={handleCreateRun}>
              ...
              <label>
                ...
                <textarea
                  ...
                  onChange={(event) => setFormState((current) => ({ ...current, domains: event.target.value }))}
              ...
              <div className="form-row">
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) => setFormState((current) => ({ ...current, mode: event.target.value as RunFormState['mode'] }))}
                  >
              ...
              {modalMessage ? <p className="status-line status-error">{modalMessage}</p> : null}
              <div className="modal-actions">
                <button type="button" className="ghost-button" onClick={() => setCreateModalOpen(false)}>
                  Cancel
                </button>
                <button type="submit" className="hero-button" disabled={busyAction === 'create-run'}>
      ...
      {tooltip.visible ? (
        <div className="live-tooltip" role="tooltip" style={{ left: tooltip.left, top: tooltip.top }}>
          {tooltip.text}
        </div>
      ) : null}
    </main>
...
function AssetsTable({
  ...
  const traceForAsset = (assetID: string) => traces.find((trace) => trace.asset_id === assetID) ?? null
    ...
    <table className={`asset-table asset-table-${tab}`}>
      <thead>
        <tr>
          {sortKeys.map((key) => (
            <th key={key}>
              <button type="button" className="sort-button" data-tooltip={assetSortTooltips[key]} onClick={() => onSortChange(key)}>
      ...
      <tbody>
        {assets.map((asset) => {
          ...
          if (tab === 'domains') {
            return <DomainAssetRow key={asset.asset_id} {...sharedProps} />
          }
          if (tab === 'ips') {
  ...
  return (
    <>
      <tr className={isExpanded ? 'is-expanded' : ''}>
        <td>
          <AssetIdentityCell asset={asset} isExpanded={isExpanded} summary={summary} onToggle={onToggle} onOpenTrace={onOpenTrace} />
        </td>
        ...
        <tr className="detail-row">
          <td colSpan={5}>
            <div id={`asset-detail-${asset.asset_id}`}>
              <AssetDetailPanel asset={asset} trace={trace} onOpenTrace={onOpenTrace} />
            </div>
          </td>
        </tr>
      ) : null}
...
function DomainAssetRow(props: SharedAssetRowProps) {
  const { asset, isExpanded, trace, onToggle, onOpenTrace, summary } = props
  ...
  return (
    <>
      <tr className={isExpanded ? 'is-expanded' : ''}>
        ...
        <td data-tooltip={tooltipIfNeeded(asset.organization || '', 32)}>{asset.organization || 'n/a'}</td>
        <td data-tooltip={tooltipIfNeeded(asset.ptr || '', 32)}>{asset.ptr || 'n/a'}</td>
        <td>
          <SourcePills value={asset.source} />
        </td>
        <td>{humanizeToken(asset.status, 'Unknown')}</td>
...
function AssetIdentityCell({
  ...
  return (
    <div className="asset-identifier-stack">
      <button
        type="button"
      ...
      <span className="table-subcopy" data-tooltip={tooltipIfNeeded(summary, 88)}>
        {summary}
      </span>
...
function AssetDetailPanel({ asset, trace, onOpenTrace }: { asset: LiveAssetRow; trace: LiveTrace | null; onOpenTrace: () => void }) {
  const detailPairs = parseAssetDetails(asset.details).filter((pair) => pair.label !== 'Ownership' && pair.label !== 'Reason')
  ...
  return (
    <div className="detail-grid">
      <article className="detail-card">
        ...
        <p className="detail-reason">{asset.inclusion_reason || 'No inclusion reason was exported for this asset.'}</p>
        <div className="badge-row">
          <span className="pill">{humanizeToken(asset.asset_type, 'Unknown')}</span>
          {asset.registrable_domain ? <span className="pill pill-subtle">{asset.registrable_domain}</span> : null}
          {asset.domain_kind ? <span className="pill pill-subtle">{humanizeToken(asset.domain_kind)}</span> : null}
          {asset.resolution_status ? <span className="pill pill-subtle">{humanizeToken(asset.resolution_status)}</span> : null}
          {asset.ptr ? <span className="pill pill-subtle">{asset.ptr}</span> : null}
          <OwnershipPill value={asset.ownership_state} />
        </div>
        ...
        <button type="button" className="ghost-button compact" onClick={onOpenTrace}>
          Open Trace View
        </button>
      </article>
      ...
      <article className="detail-card">
        ...
        {evidenceGroups.length > 0 ? (
          ...
          <p className="empty-copy">No additional evidence preview was exported for this asset.</p>
        )}
      </article>
      ...
      <article className="detail-card">
        ...
        <p className="panel-copy">
          {trace?.contributors?.length ?? 0} contributor{trace?.contributors?.length === 1 ? '' : 's'} and {related.length} related asset
        ...
        {related.length > 0 ? (
          <div className="related-list">
            {related.slice(0, 3).map((link) => (
              <span key={link.asset_id} className="pill pill-subtle" data-tooltip={link.description}>
                {link.identifier}
              </span>
            ))}
          </div>
...
function SourcePills({ value }: { value: string | undefined }) {
  ...
  return (
    <div className="source-list">
      {sources.map((source) => (
        <span key={source} className="pill source-pill tooltip-pill" data-tooltip={describeSource(source)}>
          {source}
        </span>
      ))}
    </div>
...
function OwnershipPill({ value }: { value: string | undefined }) {
  ...
  return (
    <span className={`pill ownership-pill tooltip-pill${tone}`} data-tooltip={describeOwnership(value)}>
...
function resolveTraceTargetID(view: LiveView, assetID: string, traces: LiveTrace[], assets: LiveAssetRow[]): string {
  if (view !== 'trace') {
    return assetID
  }
  const availableIDs = new Set<string>([...traces.map((trace) => trace.asset_id), ...assets.map((asset) => asset.asset_id)])
...
function buildTraceOptions(assets: LiveAssetRow[], traces: LiveTrace[]): Array<{ asset_id: string; identifier: string }> {
  const options = new Map<string, { asset_id: string; identifier: string }>()
  traces.forEach((trace) => {
    options.set(trace.asset_id, { asset_id: trace.asset_id, identifier: trace.identifier })
  })
  assets.forEach((asset) => {
    if (!options.has(asset.asset_id)) {
      options.set(asset.asset_id, { asset_id: asset.asset_id, identifier: asset.identifier })
    }
  })
  return [...options.values()].sort((left, right) => left.identifier.localeCompare(right.identifier))
}

function comparePivots(left: LivePivotRecord, right: LivePivotRecord): number {
  const scoreDiff = (right.recommendation_score ?? 0) - (left.recommendation_score ?? 0)
  if (scoreDiff !== 0) {
    return scoreDiff
  }
...
function showTooltip(element: HTMLElement, setTooltip: (updater: TooltipState | ((current: TooltipState) => TooltipState)) => void) {
...
function hideTooltip(setTooltip: (updater: TooltipState | ((current: TooltipState) => TooltipState)) => void) {
...
function sessionLabel(session: LiveAuthSession): string {
  return session.displayName?.trim() || session.email.split('@')[0] || session.email
}
...
function isCompactViewport(): boolean {
  return typeof window.matchMedia === 'function' ? window.matchMedia('(max-width: 1100px)').matches : false
}
