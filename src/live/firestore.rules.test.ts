...
describeWithEmulator('Firestore rules', () => {
  ...
  it('allows the owner to read the run and nested data', async () => {
    ...
    const ownerDB = testEnv
      .authenticatedContext('uid-owner', { email: 'analyst@zerofox.com', email_verified: true })
      .firestore()
  ...
  it('denies reads to a different user', async () => {
    ...
    const otherDB = testEnv
      .authenticatedContext('uid-other', { email: 'analyst@zerofox.com', email_verified: true })
      .firestore()
  ...
  it('denies reads to a non-allowlisted email even when the uid matches', async () => {
    ...
    const blockedDB = testEnv
      .authenticatedContext('uid-owner', { email: 'person@example.com', email_verified: true })
      .firestore()
  ...
  it('denies all client writes', async () => {
    ...
    await assertFails(setDoc(doc(ownerDB, 'runs/run-1'), { owner_uid: 'uid-owner' }))
    await assertFails(setDoc(doc(ownerDB, 'runs/run-1/assets/asset-2'), { identifier: 'blocked.example.com' }))
  })
})
